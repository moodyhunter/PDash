...
Item {
    ...
    property real inMargin: 10
    property real outMargin: 10
    ...
    property Component handleLinker: Item {
        ...
        Rectangle {
            ...
            width: parent.horizontal ? parent.width - x - root.handleSize / 2
                                       + handleWidth / 2 : handleWidth
            height: parent.horizontal ? handleWidth : parent.height - y
                                        - root.handleSize / 2 + handleWidth / 2
    ...
    readonly property bool containsMouseInsideHandle: {
        ...
        if (inMarginStartX <= inMarginEndX && inMarginStartY <= inMarginEndY)
            return isContained(pointerX, pointerY, inMarginStartX,
                               inMarginStartY, inMarginEndX, inMarginEndY)

        // Inside start/end
        return isContained(pointerX, pointerY, realStartX, realStartY,
                           realEndX, realEndY)
    }
    ...
    function isHoveringHandle(startX, startY, endX, endY) {
        return containsMouse && isContained(pointerX, pointerY, startX, startY,
                                            endX, endY)
    }
    ...
    Repeater {
        ...
        delegate: Loader {
            ...
            readonly property real realX: index === 1
                                          || index === 2 ? root.realEndX : root.realStartX
            readonly property real realY: index === 0
                                          || index === 1 ? root.realStartY : root.realEndY
